...
class CRUDPROVIDERNEWS {
    ...
    //                 try {
    //                     const isProviderNews = await this._ProviderNews.findAll({
    //                         where: {
    ...
    create(providerNewsOptions, callback) {
        let providerNews;
        let err;
        ...
        const medicatePromise = new Promise((resolve, reject) => {
            try {
                sequelize.transaction(async (t) => {
                    try {
                        const isProviderNews = await this._ProviderNews.create(providerNewsOptions, { transaction: t });
                        resolve(isProviderNews);   
                    } catch (error) {
                        reject(error);
                    }
